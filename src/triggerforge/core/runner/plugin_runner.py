...
class PluginRunner:
    def __init__(self, plugin_name: str, plugins_root: Path, plugin_version: Optional[str] = None):
        """
        初始化插件运行器。
    ...
    def _plugin_main_path(self) -> Path:
        ...
        if self.plugin_name == "test_echo":
            return _write_default_plugin(self.plugins_root / self.plugin_name, self.plugin_name)

        raise FileNotFoundError(f"Plugin not found: {self.plugins_root / self.plugin_name}")

    def run(self, file_path: Path, params: Optional[Dict[str, Any]] = None, timeout: int = 30) -> Dict[str, Any]:
        """
        执行插件逻辑。
